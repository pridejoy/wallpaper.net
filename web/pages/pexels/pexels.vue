<template>
	<view class="template-details">
		...
		<view class="tabbar footerfixed dd-glass tn-color-white" style="border-radius: 100rpx;">
			...
			<view class="action" @click="likeimg()">
				<view v-if="!islike">
					<view class="bar-icon">
	...
	export default {
		...
		onShareAppMessage() {
			return {
				...
				path: '/pages/randomgril/randomgril?shareimgid=' + this.imagesID
			}
		},
		methods: {
			getrandom() {
				...
				getRandomGril({
					...
					if (res.code === 200) {
						this.swiperList[0] = res.data.imagesURL.replace(/\\/g, "/");
			...
			likeimg() {
				linkimages(this.imagesID).then(res => { 
					if (res.code == 401) {
						uni.navigateTo({
