<template>
	<view class="basic tn-safe-area-inset-bottom"> 
		<view class="top-backgroup">
			<!-- <image src='http://imagesoss.hunji.xyz/tuniao/index_bg/basic_new.jpg' mode='widthFix' class='backgroud-image'> -->
			<image src='' mode='widthFix' class='backgroud-image'>
			...
			<view class='nav-list'>
				...
				<block v-for="(content_item, content_index) in item.list" :key="content_index">
					<navigator open-type="navigate" hover-class='none' :url="content_item.url"
						...
						:class="[ getRandomCoolBg(content_index) ]">
						<view class="icon">
...
<script>
	...
	export default {
		...
		methods: {
			...
			getBasicPageJson() {
				// getbasicpagejson().then(res => {
				//  // this.navList = res.data; 
				// })
			}
		},
		created() {
			this.getBasicPageJson()
		}
...
<style lang="scss" scoped>
	...
	.nav_title {
		...
		&--wrap {
			...
			font-weight: bold;
			background-image: url(http://imagesoss.hunji.xyz/tuniao/title_bg/title44.png); 
